...
The `Profile` column is kept in each table's header for the row format, but no
`pprof` SVG is committed to the repo: on Windows, `go tool pprof -svg`'s own
rendered "Build ID" line embeds the local machine's temp-build path
(`%TEMP%\go-build.../<pkg>.test.exe`), which is not something `-trimpath` or a
source-path fix reaches, since it names where the binary was compiled, not
where its source lives. `make profile` still writes local `.prof` files for
ad hoc `go tool pprof` use; nothing under `profiles/` is committed.

`internal/allocgate.AssertZero` skips its own check under `-cpuprofile` rather than failing
the benchmark, so a gated benchmark's timing loop still contributes samples when a profile
is taken locally. See `AssertZero`'s doc comment for why the CPU profiler's own sampling
goroutine would otherwise report a false allocation.
