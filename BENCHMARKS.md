...
were taken at. The first set was taken at `69e6e4f`, which is later than the rows it is
...
One caveat when reading a committed CPU profile: a benchmark gated by
`internal/allocgate.AssertZero` aborts at that call under `-cpuprofile`, because the CPU
profiler's own sampling goroutine allocates inside the process-wide byte window the gate
measures. Its timing loop therefore contributes no samples, and the function under it is
visible through its ungated siblings instead. See the `profile` target in the `Makefile`.
...
| Date | Commit | Change | Before | After | Profile |
|---|---|---|---|---|---|
| 2026-09-21 | (this commit) | M7: first benchmarks for the loser tree, the comparator, and Merger.Next | — | see below | [cpu](profiles/merge-cpu-69e6e4f.svg), [mem](profiles/merge-mem-69e6e4f.svg) |
...
| Date | Commit | Change | Before | After | Profile |
|---|---|---|---|---|---|
| 2026-09-19 | (this commit) | M2 baseline: record codec, reader access, blob decode, canonical hash | — | see below | [cpu](profiles/store-cpu-69e6e4f.svg), [mem](profiles/store-mem-69e6e4f.svg) |
...
| Date | Commit | Change | Before | After | Profile |
|---|---|---|---|---|---|
| 2026-09-21 | (this commit) | release-batching: cache releaseUntil so a run of records inside one window costs one clock read, not one per record | 13.14 ns/op | 1.542 ns/op | [cpu](profiles/fanout-cpu-69e6e4f.svg), [mem](profiles/fanout-mem-69e6e4f.svg) |
