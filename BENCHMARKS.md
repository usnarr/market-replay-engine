...
The `Profile` column links committed `pprof` SVGs, one CPU and one memory profile per
profiled package, generated by `make profile` and named with the short commit hash they
were taken at. The first set was taken at `8105e0c`, which is later than the rows it is
attached to: these are the project's baseline profiles, taken once the hot path was
complete, not a snapshot re-taken for every row. A later row that moves a number links
the profile taken for that change.

`internal/allocgate.AssertZero` skips its own check under `-cpuprofile` rather than failing
the benchmark, so a gated benchmark's timing loop still contributes samples to these
profiles. See `AssertZero`'s doc comment for why the CPU profiler's own sampling goroutine
would otherwise report a false allocation.
...
| Date | Commit | Change | Before | After | Profile |
|---|---|---|---|---|---|
| 2026-09-21 | (this commit) | M7: first benchmarks for the loser tree, the comparator, and Merger.Next | — | see below | [cpu](profiles/merge-cpu-8105e0c.svg), [mem](profiles/merge-mem-8105e0c.svg) |
...
| 2026-09-22 | f4662da | `RunDigest`: an optional running canonical hash in the emit loop | 54.19 ns/op (`RingWrite/no_blob`) | 65.21 ns/op (`RunDigest/delta_only`) | [cpu](profiles/fanout-cpu-8105e0c.svg), [mem](profiles/fanout-mem-8105e0c.svg) |
| 2026-09-22 | fdd0fee | the reader-side lapping protocol, measured and gated for the first time | 15.02 ns/op (`RingRead`, never lapped) | 187.1 ns/op (`RingLappedRead`) | [cpu](profiles/fanout-cpu-8105e0c.svg), [mem](profiles/fanout-mem-8105e0c.svg) |
...
| Date | Commit | Change | Before | After | Profile |
|---|---|---|---|---|---|
| 2026-09-19 | (this commit) | M2 baseline: record codec, reader access, blob decode, canonical hash | — | see below | [cpu](profiles/store-cpu-8105e0c.svg), [mem](profiles/store-mem-8105e0c.svg) |
...
| Date | Commit | Change | Before | After | Profile |
|---|---|---|---|---|---|
| 2026-09-21 | (this commit) | release-batching: cache releaseUntil so a run of records inside one window costs one clock read, not one per record | 13.14 ns/op | 1.542 ns/op | [cpu](profiles/fanout-cpu-8105e0c.svg), [mem](profiles/fanout-mem-8105e0c.svg) |
